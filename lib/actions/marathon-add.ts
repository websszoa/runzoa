...
export async function marathonAddAction(
  ...
  const images = {
    cover: (values.images_cover ?? [])
      .map((i) => (i.src ?? "").trim())
      .filter(Boolean),
    medal: (values.images_medal ?? [])
      .map((i) => (i.src ?? "").trim())
      .filter(Boolean),
    souvenir: (values.images_souvenir ?? [])
      .map((i) => (i.src ?? "").trim())
      .filter(Boolean),
    detail: (values.images_detail ?? [])
      .map((i) => (i.src ?? "").trim())
      .filter(Boolean),
  };
  ...
  const eventScale =
    values.event_scale !== "" && values.event_scale != null
  ...
  const { data: inserted, error: insertError } = await supabaseAdmin
    ...
    .insert({
      ...
      registration_price:
        registrationPrice.length > 0 ? registrationPrice : null,
      images: Object.keys(images).some(
        (k) => (images as Record<string, string[]>)[k].length > 0,
      )
        ? images
        : null,
      location,
  ...
  const marathon: Marathon = {
    ...
    registration_status:
      row.registration_status as Marathon["registration_status"],
    ...
    registration_price:
      row.registration_price as Marathon["registration_price"],
