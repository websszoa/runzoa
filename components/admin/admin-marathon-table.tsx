...
import { formatDate, formatTimeToKorean } from "@/lib/utils";
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-gray-50">
            ...
            <TableHead className="w-[130px] text-center">접수상태</TableHead>
            <TableHead className="w-[130px]">국가/지역/장소</TableHead>
            <TableHead className="w-[150px]">대회일/시간</TableHead>
            <TableHead className="w-[150px]">접수 기간/시간</TableHead>
            ...
            marathons.map((marathon, index) => (
              <TableRow key={marathon.id} className="hover:bg-gray-50">
                ...
                <TableCell className="text-muted-foreground">
                  <div>
                    {marathon.country || "-"}
                    {marathon.region ? `(${marathon.region})` : ""}
                  </div>
                ...
                <TableCell className="text-muted-foreground">
                  <div>{formatDate(marathon.event_start_at)}</div>
                ...
                <TableCell className="text-muted-foreground">
                  <div>{`${formatDate(marathon.registration_start_at)} - ${formatDate(marathon.registration_end_at)}`}</div>
                  {formatTimeToKorean(marathon.registration_start_at) && (
                    <div className="text-xs text-muted-foreground/70">
                      {formatTimeToKorean(marathon.registration_start_at)}
                    </div>
                  )}
