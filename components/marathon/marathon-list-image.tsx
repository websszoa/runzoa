...
function MarathonImageItem({
  ...
  const raw = marathon.images?.cover?.[0];
  const src = raw
    ? raw.startsWith("http") || raw.startsWith("/")
      ? raw
      : `${MARATHON_IMAGE_BASE_URL}/${raw.replace(/^\//, "")}`
    : null;
    ...
    >
      <article className="group relative overflow-hidden rounded-sm border border-slate-100 transition-all duration-500 hover:shadow-xl">
        <div className="relative aspect-4/5 overflow-hidden bg-slate-100">
          <div className="absolute inset-0 flex items-center justify-center text-slate-400 font-anyvid flex-col gap-2 text-sm">
  ...
  return (
    <div className="marathon__list__image grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
      {marathons.map((marathon, index) => (
        <MarathonImageItem key={marathon.id} marathon={marathon} index={index} />
      ))}
    </div>
  );
}
